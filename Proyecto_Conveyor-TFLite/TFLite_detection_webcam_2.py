...
import coClases_vTFLite as coClases
...
"""parser.add_argument('--modeldir', help='Folder the .tflite file is located in',
                    required=True)"""
parser.add_argument('--modeldir', help='Folder the .tflite file is located in', default='/home/pi/tflite/Sample_TFLite_model')
...
parser.add_argument('--resolution', help='Desired webcam resolution in WxH. If the webcam does not support the resolution entered, errors may occur.',
                    default='1280x720')
parser.add_argument('--edgetpu', help='Use Coral Edge TPU Accelerator to speed up detection', default=True,
                    action='store_true')
...
while True:
    try:
        valid = rfiddrew.validRFID(valid, orden_ultima)
        if valid == True:
            if (str(ordmicdrew.get_orden()) == "Iniciar"):
                noderdrew.enviarOrden(orden="INICIAR")
                ordgpiodrew.inicarConveyor(orden_ultima)
                orden_ultima = "Iniciar"
            elif (str(ordmicdrew.get_orden()) == "Parar"):
                noderdrew.enviarOrden(orden="PARAR")
                ordgpiodrew.pararConveyor(orden_ultima)
                orden_ultima = "Parar"
            ...
            for i in range(len(scores)):
                if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
                    ...
                    if (xmin<300 and xmax>900):
                        noderdrew.enviarCoincidencias(object_name=object_name, score=int(scores[i]*100), frame=frame)
                        print(f"xmin: {xmin}")
                        print(f"xmax: {xmax}")
